 ...
 * can still afford it. `columns`, when given, is a Set of column keys to keep —
 * everything else is dropped. Returns the resolved style alongside the text.
 */
function table(rows, { termWidth, linkStyle = 'auto', columns = null }) {
  ...
  const build = style => {
    ...
    const cols = columns ? all.filter(col => columns.has(col.key)) : all
    ...
    if (titleCol) titleCol.width = Math.max(20, termWidth - used - 1)

    return { cols, cells, gutter, titleWidth: titleCol ? titleCol.width : 0 }
  }

  let style = linkStyle
  if (style === 'auto') {
    const probe = build('full')
    style =
      probe.cols.some(col => col.key === 'title') && probe.titleWidth < TITLE_FLOOR
        ? 'short'
        : 'full'
  }
  ...
  const line = values =>
    cols
      .map(col => pad(values[col.key] ?? '', col.width, col.align))
      .join(' '.repeat(gutter))
      .replace(/\s+$/, '')

  const out = [
    c.dim(line(Object.fromEntries(cols.map(col => [col.key, col.header]))))
  ]

  for (const r of cells) {
    // The link text is never truncated: a clipped URL is worse than a short one.
    const values = {
      number: c.cyan(link(r.number, r.url)),
      author: r.author,
      state: r.state,
      size: r.size,
      age: r.age,
      flags: r.flags,
      title: truncate(r.title, titleWidth),
      link: style === 'off' ? '' : c.dim(link(r.link, r.url))
    }
