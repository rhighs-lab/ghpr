...
/** Numbers of open PRs in repo the user has reviewed, as a Set. */
function reviewedNumbers(repo, login) {
  const q = `repo:${repo} is:pr is:open reviewed-by:${login}`
  const res = ghJson(['api', 'search/issues', '-X', 'GET', '-f', `q=${q}`, '-F', 'per_page=100'])
  return new Set((res.items || []).map(i => i.number))
}

module.exports = { GhError, gh, ghJson, myTeams, teamMembers, currentLogin, openPullRequests, reviewedNumbers }
